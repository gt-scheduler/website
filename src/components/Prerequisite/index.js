...
import { faInfoCircle, faAngleUp, faAngleDown } from '@fortawesome/free-solid-svg-icons';
import { TermContext } from '../../contexts';
import { classes, decryptReqs } from '../../utils';
import { ActionRow } from '..';

export function Prerequisite({ course, req, option, isHeader, isEmpty, isLast }) {
  const [{ term }] = useContext(TermContext);
  const [expanded, setExpanded] = useState(true);
  const reqStyle = { fontSize: "0.9em", padding: "8px" };

  const subreqs = typeof req === 'undefined' ? [] : !req[0] ? [req] :
    typeof req[0] !== 'object' ? req.slice(1, req.length) :
    typeof req[0] === 'object' ? [req] : null;
  ...
  return (
    <div>
      ...
      {expanded && isHeader && subreqs.map((req, i) => (
        <div key={i} className={classes('divider-bottom', 'nested')}>
          <Prerequisite req={req} isLast={i === subreqs.length - 1} />
        </div>
      ))}
    </div>
  );
